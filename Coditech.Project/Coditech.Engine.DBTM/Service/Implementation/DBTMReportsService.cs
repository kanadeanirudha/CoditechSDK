    ...
    public class DBTMReportsService : BaseService, IDBTMReportsService
    {
        ...
        public GraphModel TestWiseGraphReports(int dBTMTestMasterId, long dBTMTraineeDetailId, int dBTMGraphMasterId, string graphMode, DateTime fromDate, DateTime toDate, long entityId, string userType, string centreCode, bool isMobileRequest, string typeOfRecord)
        {
            ...
            if (dBTMReportsList?.Any() == true)
            {
                ...
                if (graphMaster.XParameter == CustomConstants.Date)
                {
                ...
                else if (graphMaster.XParameter == CustomConstants.Distance)
                {
                    ...
                    else
                    {
                        ...
                        {
                            var date = dBTMReportsList.Select(x => x.CreatedDate).FirstOrDefault();
                            ...
                            XValuesList = (new[] { "0" }).Concat(values.Select(v => v.ToString())).ToArray();
                        }
                ...
                {
                    List<string> xValues = new List<string>();
                    int totalCount = dBTMReportsList.Count;
                ...
                {
                    var distinctDates = dBTMReportsList.Select(x => x.CreatedDate).Distinct().ToList();
                    int aa = distinctDates.Count;
                    List<string> xValues = new List<string>();
                    for (int count = 1; count <= aa; count++)
                    {
                        xValues.Add(count.ToString());
                    }
                    XValuesList = xValues.ToArray();
                ...
                if (XValuesList != null)
                {
                    ...
                    int colorIndex = 0;
                    var groupedReports = dBTMReportsList.GroupBy(x => x.CreatedDate).ToList();
                    var groupedReportsByDateFormat = dBTMReportsList.GroupBy(x => x.CreatedDate.ToString(CustomConstants.GraphDateFormat)).ToList();
        ...
        {
            var groupedReportsList = groupedReports as IList<IGrouping<DateTime, DBTMReportsModel>> ?? groupedReports.ToList();
            short i = 1;
            foreach (var group in groupedReportsList)
            {
                ...
                else
                {
                    foreach (var item in groupList.Where(x => yParameter.Contains(x.ParameterCode)))
                    {
                        var val = dBTMReportsList.FirstOrDefault(x => yParameter.Contains(x.ParameterCode) && x.CreatedDate == group.Key && x.Row == j)?.ParameterValue;
                        yValuesList.Add(Convert.ToDecimal(val));
                        j++;
                    }
                }

                string direction = groupList.Where(x => x.ParameterCode == "Direction").Select(x => x.Comment1).FirstOrDefault();
        ...
        {
            var groupedReportsList = groupedReports as IList<IGrouping<string, DBTMReportsModel>> ?? groupedReports.ToList();
            ...
            foreach (var group in groupedReportsList)
            {
                ...
                {
                    short count = Convert.ToInt16(groupList.Select(x => x.CreatedDate).Distinct().Count());
                    ...
                    else if (graphMaster.GraphCode == CustomConstants.BestTotalTimeVsDistanceDatewise)
                    {
                        ...
                        {
                            Color = colorPalette[colorIndex % colorPalette.Length],
                            Label = $"{groupList.FirstOrDefault().TestPerformedTime.ToString(CustomConstants.GraphDateFormat)}",
                            Data = JsonConvert.SerializeObject(yValuesList.ToArray()),
                        });
                        colorIndex++;
                    }
                    ...
                    else if (graphMaster.GraphCode == CustomConstants.BestVelocityVsDistanceDatewise)
                    {
                        yValuesList = new List<decimal>();
                        yValuesList.Add(0);
                        distance = Convert.ToDecimal(groupList.FirstOrDefault(y => (y.ParameterCode == CustomConstants.Distance || y.ParameterCode == CustomConstants.DistanceMultiplyByRow)).ParameterValue);
                        short rowCount = groupList.Max(y => y.Row);
                        for (short index = 1; index <= rowCount; index++)
                        {
        ...
        #region Assignment Wise Reports
        public DBTMReportsListModel AssignmentWiseMultipleReports(string dBTMTestMasterIds, long generalTrainerMasterId, DateTime FromDate, DateTime ToDate, bool isMobileRequest)
        {
            if (generalTrainerMasterId <= 0 || string.IsNullOrWhiteSpace(dBTMTestMasterIds))
            {
                return new DBTMReportsListModel();
            }
            ...
            foreach (var test in testList)
            {
                DBTMReportsListModel list = AssignmentWiseReports(test.DBTMTestMasterId, generalTrainerMasterId, FromDate, ToDate, isMobileRequest, false);
                if (!string.IsNullOrEmpty(list?.TestPerformedTime.ToString()))
                {
                    dataTableTestPerformedList.Add(new KeyValuePair<string, DateTime>(test.TestName, Convert.ToDateTime(list.TestPerformedTime)));
                    dataTableList.Add(new KeyValuePair<string, DataTable>(test.TestName, list.DataTable));
        ...
        private DBTMReportsListModel AssignmentWiseReports(int dBTMTestMasterId, long generalTrainerMasterId, DateTime FromDate, DateTime ToDate, bool isMobileRequest, bool isDownloadReport)
        {
            if (dBTMTestMasterId <= 0)
            {
                return new DBTMReportsListModel();
        ...
        public DBTMReportVerticalDataModel GetActivityVerticalDetails(long dBTMDeviceDataId, string typeOfRecord)
        {
            ...
            Dictionary<string, DataTable> TurnList = new Dictionary<string, DataTable>();
            ...
            BindDBTMDataVerticalFormatV2(model, testData.DBTMTestMasterId, dBTMDeviceDataId, deviceData, false);
            return model;
        }
        ...
        public DBTMReportTraineeProfileListModel GetProfileDetailsList(long generalBatchMasterId, string dBTMTraineeDetailIds, string orderBy, DateTime FromDate, DateTime ToDate)
        {
            ...
            if (list?.Count > 0)
            {
                ...
                var traineeIds = string.IsNullOrWhiteSpace(dBTMTraineeDetailIds)
                    ? new List<long>()
                    : dBTMTraineeDetailIds.Split(',').Select(s => { long.TryParse(s, out var v); return v; }).Where(v => v > 0).ToList();

                // Materialize trainer info into a lookup to avoid repeated queries during iteration
                var trainerList = (from gtat in _generalTraineeAssociatedToTrainerRepository.Table
                                   join gtm in _generalTrainerMasterRepository.Table on gtat.GeneralTrainerMasterId equals gtm.GeneralTrainerMasterId
                                   join um in _userMasterRepository.Table on gtm.EmployeeId equals um.EntityId
                                   where traineeIds.Contains(gtat.EntityId)
                ...
                foreach (var dBTMTraineeProfileModel in list)
                {
                    ...
                    if (!trainerLookup.TryGetValue(dBTMTraineeProfileModel.DBTMTraineeDetailId, out var trainerNames) || string.IsNullOrWhiteSpace(trainerNames))
                        dBTMTraineeProfileModel.TrainerName = "N/A";
                    else
                        dBTMTraineeProfileModel.TrainerName = trainerNames;
        ...
        private void BindParameterValue(List<string> listviewSequenceColumnList, List<DBTMTestWisePerformanceStandard> performanceStandardList, IGrouping<string, DBTMReportsModel> group, List<DBTMTestParameterListViewSequence> listviewSequenceColumns, DataRow newRow, bool isMobileRequest, bool isDownloadReport, bool isHigherBetter)
        {
            ...
            int ageGroupEnumId = first?.AgeGroupEnumId ?? 0;
            int genderEnumId = first?.GenderEnumId ?? 0;
        ...
        private string NumberToWords(int number)
        {
            ...
            if (number > 0)
            {
                words += number < 20 ? _unitsMap[number] : _tensMap[number / 10] + ((number % 10) > 0 ? " " + _unitsMap[number % 10] : string.Empty);
            }
            return words.Trim();
        }
        ...
        private readonly string[] _tensMap =
        {
        ...
        private void BindDBTMDataVerticalFormatV2(DBTMReportVerticalDataModel dbtmReportVerticalDataModel, int DBTMTestMasterId, long DBTMDeviceDataId, DBTMDeviceData deviceData, bool isMobileRequest = false)
        {
            string displayOn = isMobileRequest ? "OnlyMobileApp" : "OnlyWeb";
            ...
            var listviewSequenceColumns = entities.Select(x => x.FromEntityToModel<DBTMTestParameterVerticalViewSequence>()).ToList();

            Dictionary<string, DataTable> keyValuePairs = new Dictionary<string, DataTable>();
            DBTMTestMaster dBTMTestMaster = _dBTMTestMasterRepository.Table.FirstOrDefault(x => x.DBTMTestMasterId == DBTMTestMasterId);

            List<DBTMDeviceData> deviceDataList = _dBTMDeviceDataRepository.Table
                                                  .Where(x => x.TestPerformedTime.Date == deviceData.TestPerformedTime.Date
                                                          && x.PersonCode == deviceData.PersonCode
                                                          && x.TestCode == dBTMTestMaster.TestCode
                                                          && x.IsValidRecord).ToList();
            List<long> dbtmDeviceDataIds = deviceDataList.Select(x => x.DBTMDeviceDataId).ToList();
            List<DBTMReportsModel> dBTMReportsList = _dBTMDeviceDataDetailsRepository.Table.Where(x => dbtmDeviceDataIds.Contains(x.DBTMDeviceDataId)).Select(x => x.FromEntityToModel<DBTMReportsModel>()).ToList();
            ReportsListDecryption(dBTMReportsList);
            dbtmReportVerticalDataModel.ActivityDetails = new Dictionary<string, string>();
            if (dBTMReportsList.Any(x => x.ParameterCode == CustomConstants.ModeOfStart))
            {
                dbtmReportVerticalDataModel.ActivityDetails.Add("Mode Of Start", dBTMReportsList.FirstOrDefault(x => x.ParameterCode == CustomConstants.ModeOfStart).Comment1);
            }
            short turn = 1;
            string previousValue = "0";
            foreach (DBTMDeviceData item1 in deviceDataList.OrderBy(y => y.CreatedDate))
            {
                foreach (var item in dBTMReportsList.Where(x => x.DBTMDeviceDataId == item1.DBTMDeviceDataId))
                {
                    item.Weight = item1?.Weight ?? 0;
                    item.Height = item1?.Height ?? 0;
                }
                var reportsLookup = dBTMReportsList.Where(x => x.DBTMDeviceDataId == item1.DBTMDeviceDataId).GroupBy(x => $"{x.ParameterCode}-{x.Row}").ToDictionary(g => g.Key, g => g.First());
                ...
                foreach (DBTMTestParameterVerticalViewSequence col in listviewSequenceColumns)
                {
                    ...
                    if (turn == 1 && col.IsCommonColumn)
                    {
                        dbtmReportVerticalDataModel.ActivityDetails.Add(col.ParameterCode, "0");
                    }
                }
                for (short i = 1; i <= maxRecursion; i++)
                {
                    ...
                    foreach (var displayColumn in listviewSequenceColumns)
                    {
                        ...
                        {
                            var fromTo = dBTMReportsList.Where(x => x.DBTMDeviceDataId == item1.DBTMDeviceDataId).FirstOrDefault(x => x.Row == i && !string.IsNullOrEmpty(x.FromTo))?.FromTo;
                        ...
                        {
                            var lookup = dBTMReportsList.Where(x => x.DBTMDeviceDataId == item1.DBTMDeviceDataId).ToLookup(x => x.CreatedDate.ToString()).FirstOrDefault();
                            if (displayColumn.ParameterCode == CustomConstants.CompletionTime)
                                value = DBTMCustomHelper.Calculation(displayColumn.ParameterCode, displayColumn.ParameterCode, lookup, 1);
                            else
                                value = DBTMCustomHelper.Calculation(displayColumn.ParameterCode, displayColumn.ParameterCode, lookup, i);
                        }
                        else
                        {
                            if (reportsLookup.TryGetValue($"{displayColumn.ParameterCode}-{i}", out var dataDetails))
                        ...
                        if (i == 1 && displayColumn.IsCommonColumn)
                        {
                            if (turn == 1)
                            {
                                dbtmReportVerticalDataModel.ActivityDetails[displayColumn.ParameterCode] = value;
                                previousValue = value;
                            }
                            else if (dBTMTestMaster.TestOutputHigher == "LO")
                            {
                                if (Convert.ToDecimal(value) <= Convert.ToDecimal(previousValue))
                                {
                                    dbtmReportVerticalDataModel.ActivityDetails[displayColumn.ParameterCode] = value;
                                    previousValue = value;
                                }
            ...
            foreach (DBTMTestParameterVerticalViewSequence col in listviewSequenceColumns)
            {
                ...
                {
                    dbtmReportVerticalDataModel.ActivityDetails.Add(col.ColumnName.Replace("{Unit}", DBTMCustomHelper.Unit(col.ParameterCode)), $"{dbtmReportVerticalDataModel.ActivityDetails.FirstOrDefault(x => x.Key == col.ParameterCode).Value}");
                    dbtmReportVerticalDataModel.ActivityDetails.Remove(col.ParameterCode);
                }
            }
            dbtmReportVerticalDataModel.TurnList = keyValuePairs;
        }
